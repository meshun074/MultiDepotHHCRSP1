...
import static org.example.WeightedMultiDepotGA.EvaluationFunction.patientIsAssigned;
import static org.example.WeightedMultiDepotGA.GeneticAlgorithm.conflictCheck;
...
public class BestCostRouteCrossover {
    ...
    public static void initialize(InstancesClass instance) {
        ...
        numOfCaregivers = allCaregivers.length;
    ...
    private double calMoveCost(int first, int m, int second, int n, int patient, Chromosome cTemp, double bestCost, Shift[] shifts, boolean isInvalid) {
            ...
            if (routeEnd != -1) {
                ...
                for (int j = routeEnd; j < route.size(); j++) {
                    ...
                    solutionCost = patientIsAssigned(genes, i, current, route.get(j),
                            totalTravelCost, routesCurrentTime,
                            highestAndTotalTardiness, totalWaitingTime,
                            startingWaitingTime, totalOvertime, routeEndPoint, track);
        ...
        for (int i = 0; i < routeEndPoint.length; i++) {
            ...
            if (routeEnd != -1) {
                ...
                overallOvertime += Math.max(0,
                        routesCurrentTime[i] - caregiverClosingTime);
                double routeIdleTime = totalWaitingTime[i]
                        + Math.max(0, caregiverClosingTime - routesCurrentTime[i])
                        + startingWaitingTime[i];
                highestIdleTime = Math.max(highestIdleTime, routeIdleTime);
            }
        }

        return totalTravelCost + highestAndTotalTardiness[0]
                + highestAndTotalTardiness[1] + totalWaitingTime[numOfCaregivers]
                + highestIdleTime + overallOvertime;
    }
    ...
    public static void removeAffectedPatient(int iPatient, int[] routeMove, int[] positionMove, Chromosome base, int[] routeEndPoint) {
        for(int j = 0; j < routeMove.length; j++){
            markAffectedPatients(
                    iPatient, routeMove[j], positionMove[j], base, routeEndPoint);
        }
    }

    private static void markAffectedPatients(
            int excludedPatient,
            int routeIndex,
            int startPosition,
            Chromosome base,
        ...
        List<Integer> currentRoute = genes[routeIndex];
        for (int position = startPosition; position < currentRoute.size(); position++) {
            int patient = currentRoute.get(position);
            if (patient == excludedPatient
                    || allPatients[patient].getRequired_caregivers().length <= 1) {
                continue;
            }
            int otherRoute = getRouteIndexMethod(
                    routeIndex, patientToRoutesMap.get(patient));
            if (otherRoute < 0) {
                continue;
            }
            int patientIndex = genes[otherRoute].indexOf(patient);
            if (routeEndPoint[otherRoute] == -1
                    || routeEndPoint[otherRoute] > patientIndex) {
                routeEndPoint[otherRoute] = patientIndex;
                markAffectedPatients(
                        excludedPatient, otherRoute, patientIndex, base, routeEndPoint);
            }
        }
    }
