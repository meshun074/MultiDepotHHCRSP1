...
public class EvaluationFunction {
    ...
    private static int numOfDepartingPoints;
    ...
    public static void initialize(InstancesClass data) {
        ...
        numOfDepartingPoints  = dataset.getDeparting_points().length;
    ...
    public static void Evaluate(Chromosome ch) {
        ...
        if (routes[0] == null) {
            ch.initializeCaregiversRoute(dataset);
        }
        ...
        for (Shift s : routes) {
            if (s.getRoute().isEmpty()) {
                continue;
            }
            int depot = s.getCaregiver().getCacheStartingPoint();
            int lastLocationId = patientMatrixIndexes[
                    s.getRoute().get(s.getRoute().size() - 1)];
    ...
    public static boolean patientAssignment(Chromosome ch, int patient, Shift caregiver1, Shift[] routes, int i, Set<Integer> track) {
        ...
        if (p.getRequired_caregivers().length > 1) {
            ...
            if (!p.getPossibleFirstCaregiver().contains(
                    caregiver1.getCaregiver().getCacheId())
                    || !p.getPossibleSecondCaregiver().contains(
                    caregiver2.getCaregiver().getCacheId())) {
                return true;
            }
    ...
    private static void processSingleCaregiver(Chromosome ch, Shift caregiver,
                                               Patient p, int patient, double startTime, double timeWindowEnd, double waitingTime, double startingWatingTime, double travelCost, double finishingTravelCost) {
        ...
        caregiver.updateStartingWaitingTime(startingWatingTime);
    ...
    private static int findSecondCaregiver(int pIndex, int route1, Shift[] routes, Chromosome ch, Set<Integer> track) {
        ...
        for (int routeIndex = 0; routeIndex < genes.length; routeIndex++) {
            if (routeIndex != route1 && genes[routeIndex].contains(pIndex)) {
                List<Integer> route = genes[routeIndex];
            ...
            for (int i = 0; i < routeEndPoint.length; i++) {
                ...
                if (routeEnd != -1) {
                    ...
                    for (int j = routeEnd; j < route.size(); j++) {
                        int current = j == 0
                                ? routeStartingPoint
                                : patientMatrixIndexes[route.get(j - 1)];
                        solutionCost = patientIsAssigned(genes, i, current, route.get(j),
                                travelCost, routesCurrentTime, highestAndTotalTardiness,
                                TotalWaitingTime, startingWaitingTime, overtime, routeEndPoint,
                                track);
    ...
    public static double patientIsAssigned(
            List<Integer>[] genes,
            int route1,
            int curPatientIndex1,
            int nextPatientIndex,
            double totalTravelCost,
            double[] routesCurrentTime,
            double[] highestAndTotalTardiness,
            double[] totalWaitingTime,
            double[] startingWaitingTime,
            double[] overtime,
            int[] routeEndPoint,
            Set<Integer> track) {
        ...
        if (nextPatient.getRequired_caregivers().length > 1) {
            ...
            int route2 = findOtherCaregiver(nextPatientIndex, route1, genes,
                    totalTravelCost, routesCurrentTime, highestAndTotalTardiness,
                    totalWaitingTime, startingWaitingTime, overtime, routeEndPoint, track);
            ...
            if (position2 == -1) {
                ...
                curPatient2 = patientMatrixIndexes[genes[route2].get(position2)];
            }
            ...
            if (!nextPatient.getPossibleFirstCaregiver().contains(
                    caregiver1.getCacheId())
                    || !nextPatient.getPossibleSecondCaregiver().contains(
                    caregiver2.getCacheId())) {
                return Double.POSITIVE_INFINITY;
            }
    ...
    private static int findOtherCaregiver(
            int patient,
            int route1,
            List<Integer>[] genes,
            double totalTravelCost,
            ...
            if (i != route1 && genes[i].contains(patient)) {
                ...
                while (routeEndPoint[i] != patientPosition && j < route.size()) {
                    ...
                    if (j == 0)
                        ...
                        curPatient = patientMatrixIndexes[route.get(j - 1)];
                    if (patientIsAssigned(genes, i, curPatient, route.get(j), totalTravelCost,
                            routesCurrentTime, highestAndTotalTardiness, totalWaitingTime,
                            startingWaitingTime, overtime, routeEndPoint,
                            track) == Double.POSITIVE_INFINITY)
                        return Integer.MAX_VALUE;
                    j++;
                }
                return i;
    ...
    private static boolean SwapRoutes(Patient p, int route1, int route2) {
        ...
        Set<Integer> service1Routes = dataset.getQualifiedCaregiver(service1);
        Set<Integer> service2Routes = dataset.getQualifiedCaregiver(service2);
        int caregiver1Id = allCaregivers[route1].getCacheId();
        int caregiver2Id = allCaregivers[route2].getCacheId();

        return (service1Routes.contains(caregiver2Id)
                && !service2Routes.contains(caregiver2Id))
                || (service2Routes.contains(caregiver1Id)
                && !service1Routes.contains(caregiver1Id))
                || (service1Routes.contains(caregiver2Id)
                && !service1Routes.contains(caregiver1Id));
    }
}
