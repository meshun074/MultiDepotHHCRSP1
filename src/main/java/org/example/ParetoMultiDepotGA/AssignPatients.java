...
public class AssignPatients {;
    ...
    private static final ThreadLocal<double[]> routeStartTime = ThreadLocal.withInitial(() -> new double[caregivers.length]);
        ...
        if (patient.getRequired_caregivers().length > 1) {
            ...
            for (int i = 0; i < waitingTimeIndex; i++) {
                if (i == c1) {
                    ...
                    highestIdleTime = Math.max(highestIdleTime, (waitingTime2 + highestIdleTimeAndWaitingTime[i] + startingWaitingTime2));
                } else {
