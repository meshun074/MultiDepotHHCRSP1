                        ...
                        for (int n = 0; n <= secondSize; n++) {
                            ...
                            if (noEvaluationConflicts(c1Routes[first], c1Routes[second], m, n)) {
                                ...
                                if (bestCost == Double.MAX_VALUE || bestCost - tempCost > 0.001 && bestCost != Double.POSITIVE_INFINITY && tempCost != Double.POSITIVE_INFINITY || tempCost <= bestCost && rand.nextBoolean()) {
                                    ...
                                    if(tempCost != cTemp.getFitness()){
                                        System.out.println("Wht2");
                                        System.out.println(tempCost+" "+cTemp.getFitness());
                                        System.out.println("Total Distance: " + cTemp.getTotalTravelCost() + " Total Tardiness: " + cTemp.getTotalTardiness() + " Highest Tardiness: " + cTemp.getHighestTardiness() +
                                                " Total Waiting Time " + cTemp.getTotalWaitingTime() + " Total Overtime " + cTemp.getOvertime() + " Highest Idle Time " + cTemp.getHighestIdleTime());
                                        EvaluationFunction.Evaluate(cTemp);
                                        if(tempCost != cTemp.getFitness()){
